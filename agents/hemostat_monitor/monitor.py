...
class ContainerMonitor(HemoStatAgent):
    ...
    def _poll_containers(self) -> None:
        ...
        """
        try:
            containers = self.docker_client.containers.list(all=True, filters={"status": ["running", "exited"]})
            ...
            for container in containers:
                try:
                    ...
                    self.logger.error(
                        f"Error checking container {container.short_id}: {e}",
                        exc_info=False
                    )
    ...
    def _calculate_cpu_percent(self, stats: Dict[str, Any]) -> float:
        ...
        Formula: (delta_cpu / delta_system) × online_cpus × 100
